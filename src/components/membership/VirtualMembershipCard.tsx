...
const VirtualMembershipCard = ({
  ...
  return (
    <div
      className={`${
        previewMode
          ? "max-w-md w-full scale-[0.85] my-[-24px]"
          : "max-w-md w-full"
      }`}
    >
      {/* Card Container */}
      ...
      >
        ...
        <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center">
          <div className="flex items-center">
            ...
            <span className="text-white font-heading font-bold tracking-wide">
              MK PARTY
            </span>
          </div>
        ...
        <div className="absolute top-20 left-0 right-0 px-6 flex">
          ...
          <div className="flex-1">
            ...
            <div
              className={`mt-3 inline-block px-2 py-1 rounded-full text-xs border border-white/20 
              ${
                isExpired ? "bg-red-500/70 text-white" : "bg-gold/70 text-white"
              }`}
            >
              {isExpired
                ? "EXPIRED"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 pt-10 bg-gradient-to-t from-black/40 to-transparent">
          <div className="grid grid-cols-2 gap-7 text-white/90">
            <div>
              <div className="text-xs text-white/70 mb-1">Membership:</div>
              <div className="font-mono font-medium">{user.membershipType}</div>
            </div>
            <div>
